...
/* ─── Monthly sheet parser (3-row header) ──────────────────── */
/*
  CSV layout:
    Row 0: "Data for the month ending 31 May 2026"
    Row 1: "",       "Source…",   "Carried…", "Received…", "Total…", "Resolved…", "Pending at end of month", "",          "Average time…"
    Row 2: "",       "",          "",          "",           "",       "",           "Pending <3 months",       "Pending >3 months", ""
    Row 3+: data (Last Month, source rows, Grand Total)
*/
function parseMonthlySheet(rows: string[][]): SheetData {
    const period = rows[0]?.[0]?.replace(/^"|"$/g, '').trim() ?? '';

    const topHeader = rows[1].map(h => h.replace(/\n/g, ' ').replace(/\s+/g, ' ').trim());
    const subHeader = rows[2].map(h => h.replace(/\n/g, ' ').replace(/\s+/g, ' ').trim());

    const merged: string[] = topHeader.map((h, i) => {
        if (h && subHeader[i]) return `${h} · ${subHeader[i]}`;
        if (subHeader[i]) return subHeader[i];
        if (h) return h;
        return '';
    });
    ...
    const shortHeaders = [
        'SN', 'Source of complaint', 'Carried forward', 'Received',
        'Total Pending', 'Resolved', 'Pending (<3mo)', 'Pending (>3mo)', 'Avg days',
    ];

    const data = rows.slice(1).filter(r => r.some(c => c.trim().length > 0));
    console.log("data", data)

    return { headers: merged, shortHeaders, data, period };
}
...
/* ─── Types ──────────────────────────────────────────────────── */
type SheetData = { headers: string[]; data: string[][]; period?: string; shortHeaders?: string[] };
...
const MonthlySheet: React.FC<{ sheet: SheetData }> = ({ sheet }) => {
    const { headers, data, period, shortHeaders } = sheet;
    ...
    const sourceRows = data.filter(r => {
        ...
        return src.length > 0 && !src.includes('grand');
    });

    return (
        <div className="space-y-6">
            {period && (
                <p className="text-xs font-medium text-brand-saffron/80 flex items-center gap-2">
                    <Calendar size={13} />
                    {period}
                </p>
            )}
            ...
            <motion.div variants={fadeUp} initial="hidden" whileInView="visible" viewport={{ once: true }}>
                ...
                {/* desktop table */}
                <div className="hidden sm:block rounded-xl border border-gray-200">
                    <div className="overflow-x-auto">
                        <table className="w-full table-auto border-collapse text-left">
                            <thead>
                                <tr className="border-b border-gray-200 bg-gray-50">
                                    {(shortHeaders ?? headers).map((header, index) => (
                                        <th
                                            key={index}
                                            className="whitespace-nowrap px-4 py-3 text-[10px] font-bold uppercase tracking-wider text-gray-500"
                                        >
                                            {header || "—"}
                                        </th>
                                    ))}
                                ...
                                {data.map((row, rowIndex) => {
                                    ...
                                    return (
                                        <tr
                                            ...
                                            className={
                                                isGrand
                                                    ? "bg-brand-saffron/[0.05] font-semibold"
                                                    : "hover:bg-gray-50"
                                            }
                                        >
                                            {(shortHeaders ?? headers).map((_, colIndex) => {
                                                const value =
                                                    colIndex < 2
                                                        ? row[colIndex] || ""
                                                        : fmt(row[colIndex] ?? "0");
                      ...
                      ${colIndex === 0
                                                                ...
                                                                : colIndex === 1
                                                                    ? "text-gray-800"
                                                                    : isGrand
                                                                        ? "font-mono tabular-nums text-brand-saffron/90"
                                                                        : "font-mono tabular-nums text-gray-600"
                                                            }`}
                                                    >
                                                        {value}
                                                    </td>
...
const AnnualSheet: React.FC<{ sheet: SheetData }> = ({ sheet }) => {
    ...
    return (
        <div className="space-y-6">
            ...
            {yoyChange !== null && (
                <motion.div variants={fadeUp} initial="hidden" animate="visible"
                    className={`flex items-center gap-3 rounded-xl border px-5 py-4 ${yoyChange <= 0
...
const ComplaintDetails: React.FC = () => {
    ...
    const fetchSheet = useCallback(async (key: TabKey) => {
        ...
        try {
            ...
            const parsed = key === 'monthly' ? parseMonthlySheet(rows) : splitHeaderData(rows);
